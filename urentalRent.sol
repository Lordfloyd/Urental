...
contract urentalRents is Ownable{
    ...
    function rent(bytes32 _id, address payable _seller, uint256 _caution, uint256 _duration ) external payable {
        
        uint256 amount = msg.value - _caution;
        if( idExist(_id) ) {
            require( false, "this rent allready exist" );
        }
        Rent storage rentList = rents[_id];
        
        rentList.buyer = msg.sender;
        rentList.seller = _seller;
        rentList.amount = amount;
    ...
    */
    ...
    function getRent(bytes32 _id) external view returns (address, uint256, uint256, uint256, uint8) {
        return (rents[_id].seller, rents[_id].amount, rents[_id].caution, rents[_id].duration, rents[_id].stateRent);
    }
    ...
    function releaseRent(bytes32 _id) external onlyAfter(_id) onlyRentInUse(_id) returns (bool success) {
        ...
        } else if (rents[_id].stateBuyer == ACTION_BUYER_RELEASE_RENT && rents[_id].stateSeller == ACTION_SELLER_REQUEST_CAUTION) {
